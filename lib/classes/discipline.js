...
export default class Discipline {
  ...
  set offeringPeriod(rawColumn) {
    const regex = /(\d{1})\D(\d{4})/;
    if (rawColumn != undefined && rawColumn != "" && rawColumn != "N/D") {
      const period = rawColumn.match(regex);
      if (period && period.length != 0)
        this._offeringPeriod = `${period[1]}º sem. ${period[2]}`;
    }
  }

  get offeringPeriod() {
    return this._offeringPeriod;
  }
  ...
  matchesFilter({ primary, terciary }) {
    ...
    if (unity != undefined) {
      terciaryMatch = this.unity === unity;
    }
    ...
    if (period != undefined) {
      terciaryMatch = terciaryMatch && this.offeringPeriod === period;
    }
