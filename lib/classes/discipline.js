...
export class Discipline {
  ...
  set categoryEntrepreneurship(rawColumn) {
    if (rawColumn != undefined) {
      ...
      if (rawColumn.length > 0)
        this.keywords.push("Empreendedorismo");
    }
  }
  ...
  set categoryIntellectualProperty(rawColumn) {
    if (rawColumn != undefined) {
      ...
      if (rawColumn.length > 0)
        this.keywords.push("Propriedade Intelectual");
    }
  }
...
export class DisciplineGenerator {
  static run(row) {
    ...
    base.url = row[3];
    base.categoryBusiness = row[9];
    base.categoryEntrepreneurship = row[10];
    base.categoryInnovation = row[11];
    base.categoryIntellectualProperty = row[12];
