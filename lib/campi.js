const campi = [
  {
    name: "Bauru",
    unities: ["Faculdade de Odontologia de Bauru - FOB"],
  },
  {
    name: "Butantã",
    unities: [
      "Faculdade de Ciências Farmacêuticas - FCF",
      "Instituto de Química - IQ",
      "Instituto de Ciências Biomédicas - ICB",
      "Faculdade de Medicina - FM",
      "Instituto de Matemática e Estatística - IME",
      "Faculdade de Educação - FE",
      "Faculdade de Medicina Veterinária e Zootecnia - FMVZ",
      "Faculdade de Odontologia - FO",
      "Escola Politécnica - EP",
      "Faculdade de Filosofia, Letras e Ciências Humanas - FFLCH",
      "Instituto de Astronomia, Geofísica e Ciências Atmosféricas - IAG",
      "Escola de Educação Física e Esporte - EEFE",
      "Instituto de Biociências - IB",
      "Faculdade de Arquitetura e Urbanismo - FAU",
      "Faculdade de Economia, Administração e Contabilidade - FEA",
      "Escola de Comunicações e Artes - ECA",
      "Escola de Enfermagem - EE",
      "CeBiMar - Centro de Biologia Marinha",
      "CENA - Centro de Energia Nuclear na Agricultura",
      "IEA - Instituto de Estudos Avançados",
      "IEB - Instituto de Estudos Brasileiros",
      "IEE - Instituto de Energia e Ambiente",
      "IMT - Instituto de Medicina Tropical",
    ],
  },
  {
    name: "Cidade Universitária",
    unities: [],
  },
  {
    name: "Faculdade de Direito",
    unities: [],
  },
  {
    name: "Faculdade de Saúde Pública",
    unities: [],
  },
  {
    name: "Lorena",
    unities: [],
  },
  {
    name: "Medicina",
    unities: [],
  },
  {
    name: "Piracicaba",
    unities: [
      "Centro de Energia Nuclear na Agricultura - CENA",
      'Escola Superior de Agricultura "Luiz de Queiroz" - ESALQ',
      "Centro de Energia Nuclear na Agricultura – CENA",
      "Centro de Energia Nuclear na Agricultura (CENA)",
      "Escola Superior de Agricultura Luiz de Queiroz - ESALQ",
      'Escola Superior de Agricultura "Luiz de Queiroz" – ESALQ',
    ],
  },
  {
    name: "Pirassununga",
    unities: ["Faculdade de Medicina Veterinária e Zootecnia - FMVZ"],
  },
  {
    name: "Quadrilátero da Saúde",
    unities: [
      "Faculdade de Saúde Pública - FSP",
      "Faculdade de Medicina - FM",
      "Escola de Enfermagem - EE",
    ],
  },
  {
    name: "Ribeirão Preto",
    unities: [
      "Faculdade de Filosofia, Ciências e Letras de Ribeirão Preto - FFCLRP",
      "Faculdade de Economia, Administração e Contabilidade de Ribeirão Preto - FEARP",
      "Faculdade de Medicina de Ribeirão Preto - FMRP",
      "Escola de Enfermagem de Ribeirão Preto - EERP",
      "Faculdade de Ciências Farmacêuticas de Ribeirão Preto - FCFRP",
      "Faculdade de Odontologia de Ribeirão Preto - FORP",
      "Faculdade de Direito de Ribeirão Preto - FDRP",
      "Faculdade de Filosofia, Ciências e Letras de Ribeirão Preto - FFCLRP, Institutos de Estudos Avançados - IEA",
      "Faculdade de Medicina de Ribeirão Preto - FMRP / Faculdade de Odontologia de Ribeirão Preto - FORP",
      "Escola de Educação Física e Esporte de Ribeirão Preto - EEFERP",
    ],
  },
  {
    name: "Ribeirão Preto-SP",
    unities: [],
  },
  {
    name: "SanFran",
    unities: [],
  },
  {
    name: "Santos",
    unities: [],
  },
  {
    name: "São Carlos",
    unities: [
      "Escola de Engenharia de São Carlos - EESC",
      "Instituto de Ciências Matemáticas e de Computação - ICMC",
      "Instituto de Física de São Carlos - IFSC",
      "Instituto de Química de São Carlos - IQSC",
      "Instituto de Arquitetura e Urbanismo de São Carlos - IAU",
    ],
  },
  {
    name: "São Paulo",
    unities: [],
  },
  {
    name: "USP Leste",
    unities: ["Escola de Artes, Ciências e Humanidades - EACH"],
  },
  {
    name: "Todos",
    unities: [],
  },
  {
    name: "Todos os Campi",
    unities: [],
  },
];

export default campi;
