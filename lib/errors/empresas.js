...
export const findErrors = function (objects, county) {
  const knownNames = {};
  const knownCities = {};
  ...
  objects.forEach((obj, index) => {
    ...
    obj.address.city.forEach((el) => {
      if (el != "N/D") {
        ...
        let firstLetter = removeAccent(el[0]).toUpperCase();

        if (el[0] && !county[firstLetter].includes(el))
          lineErrors.push(
            `Nome ${el} de município não existe na lista de municípios do Brasil`
          );
