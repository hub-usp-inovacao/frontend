...
export const findErrors = function (objects) {
  ...
  objects.forEach((obj, index) => {
    ...
    const allEcoBelongsToFilterList = obj.ecosystems.every((eco) =>
      pluginIncubatorsList.includes(eco)
    );

    if (!allEcoBelongsToFilterList) {
      lineErrors.push("nem todas incubadoras pertencem à lista de filtros");
    }
  ...
  for (let city in knownCities) {
    let currentCity = knownCities[city];
    if (formatedKnownCities[removeDiacritics(currentCity.name)]) {
      errors.push({
        id:
          formatedKnownCities[removeDiacritics(currentCity.name)].name +
          "/" +
          currentCity.name,
        line:
          "Linhas com o primeiro padrão: \n" +
          formatedKnownCities[removeDiacritics(currentCity.name)].line.join(
            ", "
          ) +
          "\nLinhas com o segundo padrão: \n" +
          currentCity.line.join(", "),
        errors: ["Duplicação no nome da cidade"],
      });
    } else {
      formatedKnownCities[removeDiacritics(currentCity.name)] = {
        name: currentCity.name,
        line: currentCity.line,
      };
    }
  }
  return errors;
